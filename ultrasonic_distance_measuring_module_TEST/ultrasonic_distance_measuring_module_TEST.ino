...
void setup()
{
    ...
    pinMode(TrigPin, OUTPUT); /* 出力設定 */
    pinMode(EchoPin, INPUT);  /* 入力設定 */
}
...
{ 
    ...
    /* パルスを読み取り距離を算出 */
    distanse = (int((pulseIn(EchoPin, HIGH) / 58.0) * 100.0)) / 100.0;
    distanse_mm = distanse * 100;

    /* 各桁の表示する数値を算出 */
    display_01   = distanse_mm / 10   % 10; /* 1/10 の位(cm) */
    display_1    = distanse_mm / 100  % 10; /* 1    の位(cm) */
    display_10   = distanse_mm / 1000 % 10; /* 10   の位(cm) */
